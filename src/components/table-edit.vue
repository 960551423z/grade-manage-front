...
const rules: FormRules = options.list.map(item => {
  if(item.label === '分数')
    return {[item.prop]: [{ required: true, pattern: /^(\d|[1-9]\d|100)$/, message: '分数必须在1-100内', trigger: 'blur' }]}

  if(item.label === '手机号')
    return {[item.prop]: [{ required: true, pattern: /^1[3-9]\d{9}$/, message: '手机号必须是11位数字', trigger: 'blur' }]}
	if (item.required) {
